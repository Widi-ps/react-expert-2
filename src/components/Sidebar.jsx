...
function Sidebar({ threads, leaderboards, onChangeCategory }) {
  ...
  return (
    <div className='d-flex flex-column'>
      <div className='d-grid gap-2 '>
        ...
        >
          <i className='bi bi-plus-lg' /> Add New Threads
        </button>
      </div>
        ...
        && threads.map((thread) => (
          <div className='p-1' key={thread.id}>
            <button
              ...
              className={
                click === thread.id
                  ? backgrounButton.after
                  : backgrounButton.before
              }
              onClick={() => {
                CategotyClick(thread.id, thread.category);
              }}
            >
              {thread.category}
            </button>
